...
function checkEnemigs($con, $limite)
{
    ...
    $sql = mysqli_query(
        $con,
        "SELECT *, (SELECT posX FROM viajes WHERE player = players.id) as viajeX, (SELECT posY FROM viajes WHERE player = players.id) as viajeY FROM players WHERE id = '$_SESSION[player]'"
    );
    while ($row = mysqli_fetch_array($sql)) {
        $enemig = null;
    ...
    $sql = mysqli_query(
        $con,
        "SELECT *, (SELECT posX FROM viajes WHERE player = players.id) as viajeX, (SELECT posY FROM viajes WHERE player = players.id) as viajeY FROM players WHERE posX >= '$minimoX' AND $posY >= '$minimoY' AND posX <= '$maximoX' AND posY <= '$maximoY' AND ban <= '$ahora' AND fog = '0' AND NOT(id = '$_SESSION[player]')"
    );
    while ($row = mysqli_fetch_array($sql)) {
        $enemig = null;
        $enemig["nick"] = $row["nick"];
        ...
        if ($row["posX"] - $posX - 100 >= 0 and $row["posY"] - $posY >= 0) {
            $cara = 90 * 0;
            $grados =
                $cara + (90 - rad2deg(abs(atan(abs(($row["posY"] - $posY) / (abs($row["posX"] - $posX) + 0.0001))))));
        }
